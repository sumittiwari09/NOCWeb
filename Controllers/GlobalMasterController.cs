...
{
    public class GlobalMasterController : Controller
    {
        ...
        public ActionResult Index(string appNo)
        {
            ...
            if (userdetailsSession != null)
            {
                ...
                ViewBag.Course = GetYear();
                
                ViewBag.AllData = GblMstGetData();
            }
        ...
        public ActionResult SaveData(tblGlobalMaster mapping)
        {
            var userdetailsSession = (UserModelSession)Session["UserDetails"];
            var Token = Session["Token"];
            var json = JsonConvert.SerializeObject(mapping);
            var client = new RestClient(ConfigurationManager.AppSettings["BaseUrl"] + "User/SaveGlobalMst");
            var request = new RestRequest(Method.POST);
            request.AddHeader("cache-control", "no-cache");
        ...
        public List<tblGlobalMaster> GblMstGetData()
        {

            var client = new RestClient(ConfigurationManager.AppSettings["BaseUrl"] + "User/GblMstGetData");
            var request = new RestRequest(Method.POST);
            request.AddHeader("cache-control", "no-cache");
            //request.AddHeader("authorization", "bearer " + CurrentSessions.Token + "");
            request.AddParameter("application/json", "", ParameterType.RequestBody);
            ...
            request.AddHeader("Accept", "application/json");
            IRestResponse response = client.Execute(request);
            List<tblGlobalMaster> data = new List<tblGlobalMaster>();
            if (response.StatusCode.ToString() == "OK")
            {
                objResponse = JsonConvert.DeserializeObject<ResponseData>(response.Content);
                if (objResponse.Data != null)
                {
                    data = JsonConvert.DeserializeObject<List<tblGlobalMaster>>(objResponse.Data.ToString());
                }
            }
            return data;
        }
